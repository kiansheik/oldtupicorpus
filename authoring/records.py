    ...
    def from_dict(
        ...
        *,
        ...
        return cls(
            ...
            locations=tuple(
                PhilologicalLocation.from_dict(item) for item in raw_locations
            ),
            notes=tuple(notes),
...
def records_from_legacy_text(
    text: str, *, source: str, kind: str
...
def load_records(path: Path, *, source: str, kind: str) -> list[GroundTruthRecord]:
    ...
    for physical_line, raw in enumerate(
        ...
        if record.id in seen_ids:
            raise ValueError(
                f"Duplicate ground-truth record id {record.id!r} in {path}."
            )
        if record.ordinal in seen_ordinals:
            raise ValueError(
                f"Duplicate ground-truth ordinal {record.ordinal} in {path}."
            )
...
def load_preferred_records(
    ...
    return (
        records_from_legacy_text(
            legacy.read_text(encoding="utf-8"), source=source, kind=kind
        ),
        None,
    )
...
def replace_record_surface(
    ...
    for index, record in enumerate(updated):
        ...
        if record.normalized_target is not None:
            updated[index] = replace(record, normalized_target=normalized)
        else:
            updated[index] = replace(record, surface=normalized)
        return updated
