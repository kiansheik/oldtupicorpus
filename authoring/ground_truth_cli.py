...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify and regenerate Old Tupi ground truth from annotated source code."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command in {"regenerate", "migrate"}:
    ...
    if args.json:
        ...
        for result in results:
            ...
            if "stale" in result:
                print(
                    f"  source annotations differ from generated artifacts at record "
                    f"{result['stale']['ordinal']}."
                )
