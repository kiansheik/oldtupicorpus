...
BLOCKED_CALLS = frozenset({"__import__", "breakpoint", "compile", "delattr", "eval", "exec", "exit", "getattr", "globals", "help", "input", "locals", "open", "quit", "setattr", "vars"})
...
def get_source_context(source: str, record_id: str | int, *, radius: int = 3) -> dict[str, Any]:
...
def render_candidate(source: str, expression: str, *, record_id: str | int | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        return {"source": source, "expression": expression, "error": f"{exc.__class__.__name__}: {exc}"}
    ...
    if record_id is not None:
        ...
        result.update({"record": serialize_record(record), "matches_target": rendered == record.expected_surface, "target": record.expected_surface})
    return result
...
def search_rendered_expressions(query: str, *, limit: int = 30) -> list[dict[str, Any]]:
    ...
    for case in load_ground_truth_cases(include_synthetic=False):
        ...
        for ordinal, rendered in enumerate(render_lines(case.expressions), start=1):
            ...
            haystack = " ".join(item for item in (rendered, record.expected_surface if record else "", record.translation if record else "", record.analysis if record else "") if item)
...
def search_lexicon(query: str, *, limit: int = 50) -> list[dict[str, Any]]:
    ...
    for name, value in sorted(load_lexicon_namespace().items()):
        ...
        if needle not in normalize_query(f"{name} {surface} {definition or ''}"):
            continue
        results.append({"name": name, "surface": surface, "definition": str(definition) if definition else None, "type": type(value).__name__})
...
def verify_ground_truth(source: str | None = None) -> dict[str, Any]:
    ...
    for case in cases:
        try:
            ...
            blocked += 1
            outcomes.append({"source": case.name, "ok": False, "error": f"{exc.__class__.__name__}: {exc}"})
            continue
        if comparison.has_mismatch:
            blocked += 1
            outcomes.append({"source": case.name, "ok": False, "mismatch": {"ordinal": comparison.mismatch_line_no, "expected": comparison.mismatch_expected, "actual": comparison.mismatch_actual}, "extra_lines": comparison.extra_lines})
        else:
            outcomes.append({"source": case.name, "ok": True, "records": len(comparison.expected_lines), "rendered": len(comparison.actual_lines), "extra_lines": comparison.extra_lines})
...
def validate_candidate_expression(expression: str) -> None:
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.Call) and callable_name(node.func) in BLOCKED_CALLS:
            raise CandidateSafetyError(f"Candidate may not call {callable_name(node.func)}().")
...
def render_value(value: object) -> str | None:
    ...
    if not callable(evaluator):
        return None
    for attempt in (lambda: evaluator(), lambda: evaluator(False), lambda: evaluator(annotated=False)):
        try:
            rendered = attempt()
        except TypeError:
            continue
