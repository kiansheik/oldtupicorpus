...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    for name, help_text in (
        (
            "verify",
            "Verify renderings and ensure JSONL artifacts match source annotations.",
        ),
...
def verify(cases) -> tuple[int, list[dict[str, object]]]:
    ...
    for case in cases:
        ...
        if stale_ordinal is not None:
            failures += 1
            results.append(
                {"source": case.name, "ok": False, "stale": {"ordinal": stale_ordinal}}
            )
            continue
...
def regenerate(cases) -> int:
    for case in cases:
        try:
            ...
            return 1
        print(
            f"[ground-truth] {case.name}: regenerated {len(records)} record(s)\n  JSONL: {structured}"
        )
    return 0
...
def review(cases) -> int:
    ...
    for result in results:
        if result["ok"]:
            print(
                f"[ground-truth] {result['source']}: source annotations and JSONL are current"
            )
            continue
        ...
        if "stale" in result:
            ...
            print(
                "  Edit the source entry and its `# @...` comments, then run regenerate."
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        for result in results:
            ...
            if "stale" in result:
                print(
                    f"  source annotations differ from generated JSONL at record {result['stale']['ordinal']}."
                )
